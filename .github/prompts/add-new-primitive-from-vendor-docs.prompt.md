...
Use [packages/core/config/primitives.json](../../packages/core/config/primitives.json), [packages/core/config/assistants.json](../../packages/core/config/assistants.json), and [docs/configuration.md](../../docs/configuration.md) as the source of truth for the current model.
...
3. If a new primitive is justified, add it to `packages/core/config/primitives.json` with:
...
4. Update `packages/core/config/assistants.json` if assistant metadata links should also change as part of the same documentation update.
