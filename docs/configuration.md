...
| Assistant | Official availability | Official repo-scoped resources | Current scanner patterns | Notes | Sources |
|------|------|-------------|-------------|-------|---------|
| GitHub Copilot | Yes | `.github/agents/*.agent.md` (formal format); `.claude/agents/*.md` (Claude format, also loaded by VS Code) | `.github/agents/*.agent.md`; `.claude/agents/*.md` | VS Code now explicitly lists `.claude/agents/` as a workspace agent location under "Claude format". The scanner counts both the formal `.agent.md` suffix from `.github/agents/` and the Claude-format `.md` files from `.claude/agents/`. | [Custom agents](https://code.visualstudio.com/docs/copilot/customization/custom-agents) |
...
| Assistant | Official availability | Official repo-scoped resources | Current scanner patterns | Notes | Sources |
|------|------|-------------|-------------|-------|---------|
| GitHub Copilot | Yes | `.github/hooks/*.json`; `.claude/settings.json`; `.claude/settings.local.json` (Claude format, also loaded by VS Code); agent-scoped `hooks` field in `.agent.md` files | `.github/hooks/*.json`; `.claude/settings.json`; `.claude/settings.local.json` | VS Code now explicitly lists `.claude/settings.json` and `.claude/settings.local.json` as "Workspace (Claude format)" hook file locations in the default `chat.hookFilesLocations` setting. The scanner counts all three. Agent-scoped hooks in `.agent.md` frontmatter are documented but require frontmatter parsing, which the scanner does not perform. `.claude/settings.local.json` is typically gitignored; file-presence is a proxy only. | [Hooks](https://code.visualstudio.com/docs/copilot/customization/hooks), [Custom agents](https://code.visualstudio.com/docs/copilot/customization/custom-agents) |
