...
| Assistant | Official availability | Official repo-scoped resources | Current scanner patterns | Notes | Sources |
|------|------|-------------|-------------|-------|---------|
| GitHub Copilot | Yes | `.github/agents/*.agent.md` (formal format); VS Code also detects `.md` files in `.github/agents`; `.claude/agents/*.md` (Claude format, explicitly documented as an alternative workspace location) | `.github/agents/*.agent.md`; `.claude/agents/*.md` | The scanner now follows both documented workspace locations: the formal `.github/agents` folder (preferring `.agent.md` suffix) and the `.claude/agents` folder, which VS Code explicitly lists as an alternative workspace-scope location. | [Custom agents — Custom agent file locations](https://code.visualstudio.com/docs/copilot/customization/custom-agents#_custom-agent-file-locations) |
...
| Assistant | Official availability | Official repo-scoped resources | Current scanner patterns | Notes | Sources |
|------|------|-------------|-------------|-------|---------|
| GitHub Copilot | Yes | `.github/hooks/*.json`; `.claude/settings.json`; `.claude/settings.local.json` (Claude format, included in VS Code's default `chat.hookFilesLocations`); agent-scoped `hooks` field in `.agent.md` files | `.github/hooks/*.json`; `.claude/settings.json`; `.claude/settings.local.json` | The scanner now counts both the native `.github/hooks/*.json` location and the Claude-format settings files, which VS Code includes in its default `chat.hookFilesLocations`. Agent-scoped hooks in `.agent.md` frontmatter are documented but not parsed by the current scanner. `.claude/settings.local.json` is gitignored by default in Claude Code projects and thus unlikely to appear in most repository scans, but VS Code explicitly lists it as a workspace hook source. | [Hooks — Hook file locations](https://code.visualstudio.com/docs/copilot/customization/hooks#_hook-file-locations) |
